...
class TestCashRegime:
    ...
    def test_cash_too_small_buys_nothing_and_waits(self, cto, mocker):
        ...
        # Then no order is produced, and the whole cash is carried over
    ...
    def test_full_cash_buys_the_share_and_carries_the_change(self, cto, mocker):
    ...
    def test_a_fractional_asset_keeps_its_euro_amount(self, cto, mocker):
        ...
        self._with_cash(mocker, 320.0)
        envelope = _by_name(allocation.plan())["CTO"]
        # Then the whole cash goes in, no rounding involved
...
class TestProjection:
    ...
    def test_an_envelope_without_cash_repeats_the_same_split(self, cto, mocker):
        ...
        for m in months:
            assert m.envelopes[0].assets[0].amount == 100.0
            assert m.envelopes[0].carry == 0.0

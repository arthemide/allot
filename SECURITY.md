...
We will acknowledge receipt within 48 hours and keep you informed of the
progress toward a fix. Please give us a reasonable amount of time to address the
issue before any public disclosure.

## Best practices for contributors

- **Never commit secrets** (`.env` files, API keys, tokens, private keys). Keep
  them out of version control via `.gitignore`.
- **Rotate credentials** immediately if they are accidentally exposed.
- **Keep dependencies up to date** and review security advisories for the
  packages you use.

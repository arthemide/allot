...
class DCAExecutor:
    ...
    def should_execute_purchase(self) -> Tuple[bool, str]:
        ...
        base_asset = self.dca_config.base_asset
        ...
        try:
            ...
            if len(klines) < 3:
                logger.warning(f"Not enough historical data (got {len(klines)} klines, need 3)")
            ...
            logger.info(f"Period -2 (oldest): open={period_1.open}, close={period_1.close}, positive={period_1_positive}")
            logger.info(f"Period -1 (recent): open={period_2.open}, close={period_2.close}, positive={period_2_positive}")
            ...
            price_above_prum = current_price > prum * Decimal('1.03')  # Adding 3% buffer to avoid frequent skips
            
            logger.info(f"Decision factors: both_periods_positive={both_periods_positive}, price_above_prum={price_above_prum}")
            ...
            if not both_periods_positive:
                reason = f"Execute: at least one negative period (non-bullish momentum)"
            elif not price_above_prum:
                reason = f"Execute: price below or equal to PRUM ({current_price} <= {prum})"
            else:
            ...
            logger.warning("Error during decision logic - executing by default for safety")
    ...
    def check_and_ensure_balance(self, required_amount: Decimal) -> bool:
        ...
        logger.info(f"Checking funds in Simple Earn...")
        
        try:
            # Retrieve flexible position for USDC
            earn_position = self.client.get_flexible_product_position_by_asset(quote_asset)
            ...
            logger.info(f"New spot balance after transfer: {new_free_balance} {quote_asset}")
            
            if new_free_balance >= required_amount:
                logger.info("Sufficient balance after transfer from Earn")
                return True
            else:
                logger.error(f"Still insufficient balance after transfer: {new_free_balance}")
                return False
    ...
    def execute_dca_purchase(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if not self.check_and_ensure_balance(amount_usdc):
                ...
                get_notifier().notify_error("Insufficient Balance", "DCA purchase cancelled due to insufficient funds.")
                return None
            ...
            logger.info(f"Executing MARKET BUY order for {amount_usdc} {self.dca_config.quote_asset}...")
            
            order_result = self.client.create_market_order(
                symbol=symbol,
                side='BUY',
                quote_order_qty=str(amount_usdc)
            )
            ...
            logger.info(f"Quantity purchased: {executed_qty} {self.dca_config.base_asset}")

...
def test_earn_positions(client: BinanceClient, asset: str = "USDC"):
    ...
    try:
        ...
        if position:
            ...
            print(f"   ℹ️  Helper returned None (no position or zero balance)")
...
def test_symbol_info(client: BinanceClient, symbol: str = "ETHUSDC"):
    ...
    try:
        ...
        print(f"✅ Symbol information retrieved")
...
def test_prum_calculation(client: BinanceClient, symbol: str = "ETHUSDC", base_asset: str = "ETH"):
    ...
    try:
        ...
        print(f"   - Bot Total Quantity: {stats.get('transaction_total_quantity', '0')}")
        ...
        if prum is None:
            ...
            if total_balance == '0' or float(total_balance) == 0:
                ...
                print(f"   No purchase history in tracker despite having balance")
                print(f"   💡 Set DCA_BASE_PRUM and DCA_BASE_QUANTITY in .env to track existing position")
            return True
        ...
        print(f"   Price above PRUM (+3% buffer): {'Yes ⬆️' if price_above_prum else 'No ⬇️'}")
...
def test_order_history(client: BinanceClient, symbol: str = "ETHUSDC"):
    ...
    try:
        ...
        if not orders:
            ...
            print(f"   This is expected if you haven't made any purchases yet")
            return True
        
        # Count orders by type
        buy_orders = sum(1 for o in orders if o['side'] == 'BUY' and o['status'] == 'FILLED')
        sell_orders = sum(1 for o in orders if o['side'] == 'SELL' and o['status'] == 'FILLED')
        ...
        if len(orders) > 0:
            ...
            for order in orders[-3:]:
                ...
                print(f"   {side_emoji} {order['side']} - Qty: {order['executedQty']} - Status: {order['status']}")

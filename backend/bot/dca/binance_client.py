...
class BinanceClient:
    ...
    def get_asset_balance(self, asset: str) -> AssetBalance:
        ...
        for balance in account_info.get("balances", []):
            if balance["asset"] == asset:
                ...
                logger.debug(
                    f"Balance {asset}: free={free}, locked={locked}, total={total}"
                )

                return AssetBalance(free=free, locked=locked, total=total)
    ...
    def create_market_order(
        ...
        logger.info(
            f"Order executed: orderId={result.get('orderId')}, "
    ...
    def get_simple_earn_flexible_position(
        ...
        logger.debug(
            f"Retrieving Simple Earn flexible positions{' for ' + asset if asset else ''}"
        )
        result = self._request(
            "GET", "/sapi/v1/simple-earn/flexible/position", signed=True, params=params
        )
        ...
        return [
            FlexiblePosition(
                ...
                yesterday_real_time_rewards=Decimal(row["yesterdayRealTimeRewards"])
                if row.get("yesterdayRealTimeRewards")
                else None,
                accumulated_rewards=Decimal(row["accumulatedRewards"])
                if row.get("accumulatedRewards")
                else None,
    ...
    def redeem_flexible_product(self, product_id: str, amount: str) -> RedeemResponse:
        ...
        logger.info(
            f"Redeeming flexible product: productId={product_id}, amount={amount}"
        )
        result = self._request(
            "POST", "/sapi/v1/simple-earn/flexible/redeem", signed=True, params=params
        )

        return RedeemResponse(
            redeem_id=result.get("redeemId"), success=result.get("success", True)
        )

    def get_flexible_product_position_by_asset(
        self, asset: str
    ) -> Optional[FlexiblePosition]:
        """

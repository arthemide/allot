...
class Config:
    ...
    def _validate(self):
        ...
        if not self.binance.api_key or not self.binance.api_secret:
            raise ValueError("Binance API keys are required (BINANCE_API_KEY and BINANCE_API_SECRET)")
    
    def __str__(self) -> str:
        """String representation of configuration (without secrets)"""
        schedule_info = f"Days of month: {self.dca.days_of_month}" if self.dca.days_of_month else "Every day"

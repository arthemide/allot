...
wherever it is pasted. The note is in French, matching the specified format;
...
def title(today: date) -> str:
    return f"Point patrimoine - {MONTHS[today.month - 1]} {today.year}"
...
def _asset_line(asset: dict, box: str = "[ ] ") -> str:
    """What to buy, and nothing else: the reasoning is not a to-do."""
    if asset["units"]:
        parts = "part" if asset["units"] == 1 else "parts"
        return (
            f"{box}{asset['symbol']:<9} {asset['units']} {parts}"
    ...
    return line if asset["price"] else f"{line}  cours indisponible"


def _waiting_lines(envelope: dict, today: date) -> list[str]:
    ...
    """
    lines = [
        f"{envelope['envelope']} - rien ce mois. {_money(envelope['budget'])} en cagnotte."
    ]

    target = min(envelope["waiting"], key=lambda m: m["price_eur"])
    if target["missing"] <= 0:
        return lines
    ...
    lines.append(
        f"Il manque {_money(target['missing'])} pour 1 {target['symbol']}{tail}."
    )
    return lines
...
def _stale_warning(today: date) -> str | None:
    ...
    if (today - last_date).days <= STALE_DAYS:
        return None
    return f"Rien de saisi depuis le {last_date.day} {MONTHS[last_date.month - 1]}."


def _spendable(asset: dict) -> bool:
...
def render(base_url: str | None = None, today: date | None = None) -> str:
    ...
    lines.append(f"{_money(total)} à placer ce mois.")
    ...
    for envelope in envelopes:
        ...
        if not buying:
            if envelope["waiting"]:
                ...
                lines.append(
                    f"{envelope['envelope']} - {_money(envelope['budget'])} sans "
                    f"destination, aucun actif à alimenter."
                )
            else:
                lines.append(f"{envelope['envelope']} - rien ce mois.")
            continue

        if envelope["cash"]:
            lines.append(
                f"{envelope['envelope']} - {_money(envelope['budget'])} en cagnotte"
            )
        else:
        ...
        if envelope["carry"] >= 1:
            lines.append(f"Reste {_money(envelope['carry'])} en cagnotte.")
...
def _projected(month: date, entries: list[dict], base_url: str | None) -> str:
    """A month that has not happened yet, at today's prices."""
    lines = ["Prévu au rythme actuel, aux cours du jour."]
    for entry in entries:
        buying = [a for a in entry["assets"] if _spendable(a)]
        if not buying:
            continue
        lines.append("")
        label = " en cagnotte" if entry["tracked"] else ""
        lines.append(f"{entry['envelope']} - {_money(entry['budget'])}{label}")
        for asset in buying:
            lines.append(_asset_line(asset, box="  "))
    if len(lines) == 1:
        lines.append("")
        lines.append("Rien d'achetable ce mois-là : la cagnotte continue de monter.")
    if base_url:
        lines.append("")
...
def feed(
    ...
    for step in allocation.projection(today, FEED_MONTHS - 1):
        month = step["month"]
        events += _event(
            month, title(month), _projected(month, step["envelopes"], base_url), stamp
        )

    lines = [
        "BEGIN:VCALENDAR",
        "VERSION:2.0",
        "PRODID:-//Allot//Monthly note//FR",
        "CALSCALE:GREGORIAN",
        "METHOD:PUBLISH",
        "X-WR-CALNAME:Allot",
        *events,
